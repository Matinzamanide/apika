...
const CartItem: React.FC<ICartItem> = ({ id }) => {
  ...
  useEffect(() => {
    axios(`http://localhost/apitak/get_products.php?id=${id}`).then((res) => {
  ...
  const hasDiscount =
    product.before_discount_price > product.price && product.before_discount_price > 0;
  const discountPercent = hasDiscount
    ? Math.round(
        100 - (product.price / product.before_discount_price) * 100
      )
    : 0;
