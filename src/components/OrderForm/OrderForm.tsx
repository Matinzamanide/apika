...
const OrderForm = ({ totalPrice, totalOldPrice }: { totalPrice: number; totalOldPrice: number }) => {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const res = await axios.post("http://localhost/apitak/orders/submit_order.php", {
  ...
  return (
    <div className="mt-10 max-w-3xl mx-auto bg-white p-6 rounded-lg shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <input name="province" placeholder="استان" onChange={handleChange} className="border p-2 rounded" />
        <input name="city" placeholder="شهر" onChange={handleChange} className="border p-2 rounded" />
        <input name="postalCode" placeholder="کد پستی" onChange={handleChange} className="border p-2 rounded" />
        <input name="address" placeholder="آدرس کامل" onChange={handleChange} className="border p-2 rounded col-span-2" />
        <input name="phone" placeholder="شماره ثابت" onChange={handleChange} className="border p-2 rounded" />
        <select name="shippingMethod" onChange={handleChange} className="border p-2 rounded">
