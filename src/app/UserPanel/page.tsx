...
interface IOrder {
  ...
  address: string;
  postal_code: string;
  products: string; // this is still a JSON string of products
}
...
const UserPanel = () => {
  ...
  useEffect(() => {
    ...
    // گرفتن اطلاعات کاربر
    setLoadingUser(true);
    axios(`http://localhost/apitak/auth/get_user_info.php?phone=${phoneNumber}`)
      .then((result) => {
  ...
  useEffect(() => {
    ...
    const url = `http://localhost/apitak/orders/submit_order.php?user_phone=${phoneNumber}`;
