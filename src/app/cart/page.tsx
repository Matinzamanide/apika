...
const Cart = () => {
    ...
    const { isLoggedIn } = useAuthContext(); // Get isLoggedIn from AuthContext

    // گرفتن داده‌های محصولات
    useEffect(() => {
        const fetchData = async () => {
            const promises = cartItems.map((item) =>
                axios(`http://localhost/apitak/get_products.php?id=${item.id}`).then((res) => {
    ...
    // Handle continue checkout logic
    const handleContinueCheckout = () => {
        if (!isLoggedIn) {
            setLoginMessage("برای ادامه فرآیند خرید، ابتدا باید وارد حساب کاربری خود شوید.");
        } else {
            setLoginMessage(null); // Clear message if user logs in later
            setShowForm(true);
        }
    };
            ...
            {cartItems.length === 0 ? (
                ...
                <div className="max-w-5xl mx-auto space-y-6">
                    ...
                    <div className="text-right mt-10 space-y-2 border-t pt-4">
                        ...
                        {loginMessage && ( // Conditionally display login message
                        ...
                        {!showForm && (
                            <button
                                onClick={handleContinueCheckout} // Use the new handler
...
//       const promises = cartItems.map((item) =>
//         axios
//           .get(`http://localhost/apitak/get_products.php?id=${item.id}`)
//           .then((res) => {
//             const data = Array.isArray(res.data) ? res.data[0] : res.data;
//             return {
//               id: item.id,
